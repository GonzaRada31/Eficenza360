...
export class OutboxService {
  ...
  async processOutbox() {
    try {
      ...
      const processedIds = await this.prisma.$transaction(
        ...
        `;
          ...
          for (const event of events) {
            ...
            if (queueName) {
              ...
              const mq =
                (queues as any)[this.toCamelCase(queueName)] ||
                queues.deadLetter;
              ...
              this.logger.warn(
                `No queue mapping found for ${event.event_type}, sending to dead-letter`,
              );
