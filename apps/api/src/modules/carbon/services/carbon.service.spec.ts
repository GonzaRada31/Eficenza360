    ...
    tenantClient: {
      energyAudit: {
        findUnique: jest.fn().mockResolvedValue({
          ...
          year: 2026,
          carbonActivities: [
            { id: 'act-1', activityType: 'electricity', activityValue: 100 },
          ],
        }),
      },
    },
    carbonCalculation: {
      create: jest.fn().mockResolvedValue({ id: 'calc-1' }),
    },
    carbonReport: {
      create: jest.fn().mockResolvedValue({ id: 'rep-1', totalEmissions: 50 }),
    },
    domainEventOutbox: { create: jest.fn() },
  };
  const mockOccService = {
    processActivity: jest
      .fn()
      .mockReturnValue({ emissions: 50, unit: 'kgCO2e' }),
    aggregateEmissions: jest.fn().mockReturnValue(50),
  };
  const mockEmissionFactorService = {
  ...
  it('should calculate carbon footprint and emit event', async () => {
    ...
    expect(mockPrismaService.domainEventOutbox.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ eventType: 'CARBON_CALCULATED' }),
      }),
    );
