...
async function bootstrap() {
  ...
  try {
    ...
    const validationResult = await energyAuditService.validateAudit(
      tenant.id,
      audit.id,
    );
    console.log(
      `Validación Exitosa. SnapshotId: ${validationResult.snapshotId}`,
    );
    ...
    if (outboxRecords.length > 0) {
      ...
      console.log(
        `Status DB: ${outbox.status}, Payload UUID: ${(outbox.payload as any).eventId}`,
      );
    ...
    await prisma.energyAuditSnapshotRecord.deleteMany({
      where: { snapshot: { tenantId: tenant.id } },
    });
    await prisma.energyAuditSnapshot.deleteMany({
