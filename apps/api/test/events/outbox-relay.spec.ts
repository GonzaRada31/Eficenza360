...
describe('Outbox Relay (Integration)', () => {
  ...
  beforeEach(() => {
        ...
        $transaction: jest.fn(async (cb) => {
            return cb({
                $queryRaw: jest.fn().mockResolvedValue([
                    { id: '1', event_type: 'AUDIT_CREATED', tenant_id: 't1', aggregate_type: 'Audit', aggregate_id: 'a1', payload: {}, created_at: new Date() }
                ])
            });
        })
    };
    ...
    // Mock BullMQ add
    jest.spyOn(queues.auditEvents, 'add').mockResolvedValue({ id: 'job-1' } as any);
  });
  ...
  it('should lock rows, relay to right queue, and mark as processed', async () => {
    ...
    expect(queues.auditEvents.add).toHaveBeenCalledWith('AUDIT_CREATED', expect.objectContaining({ eventId: '1' }));
